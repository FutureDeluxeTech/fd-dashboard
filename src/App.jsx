    ...
    <div>
      ...
      <div style={{ background: "rgba(216,90,48,0.08)", border: "1px solid rgba(216,90,48,0.2)", borderRadius: 10, padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13, lineHeight: 1.6, color: "rgba(255,255,255,0.65)" }}>
          <p style={{ margin: 0 }}><span style={{ color: "#AFA9EC", fontWeight: 500 }}>Creative decline is real</span> — from {data[0]?.avgCreative.toFixed(2)} ({years[0]}) to {data[data.length - 1]?.avgCreative.toFixed(2)} ({years[years.length - 1]}). The drop is in alignment and quality, not execution. The team delivers well; they're getting less ambitious briefs.</p>
          <p style={{ margin: 0 }}><span style={{ color: "#EF9F27", fontWeight: 500 }}>Margin has been volatile</span> — ranging from {Math.min(...data.map(d => d.avgMargin)).toFixed(1)}% to {Math.max(...data.map(d => d.avgMargin)).toFixed(1)}% across the period. Loss-making projects peaked at {Math.max(...data.map(d => d.negMargin))}% before recovering.</p>
          <p style={{ margin: 0 }}><span style={{ color: "#D4537E", fontWeight: 500 }}>"Both high" is collapsing</span> — from {data[0]?.bothHigh}% ({years[0]}) to {data[data.length - 1]?.bothHigh}% ({years[years.length - 1]}). Projects are increasingly either creative or commercial, not both.</p>
          <p style={{ margin: 0 }}><span style={{ color: "#5DCAA5", fontWeight: 500 }}>Bigger projects work</span> — £200k+ consistently delivers both higher creative scores and better margins. Rev/project has grown from {fmt(data[0]?.revPerProject)} to {fmt(data[data.length - 1]?.revPerProject)}.</p>
        </div>
      </div>
    </div>
  );
